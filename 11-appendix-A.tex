...
\frac{N}{n(n+N)}.
\]
If one writes \(\rho_{Y,f}=\Corr(Y,f)\) and \(r=\frac{n}{N}\), then the same expression becomes
\[
\Var(\widehat{\theta}_{\lambda^\star})
=
\frac{\Var(Y)}{n}
...
\Var(\widehat{\theta}_{\lambda^\star})
\approx
\frac{\Var(Y)}{n}(1-\rho_{Y,f}^2),
\]
which is the basis of the Chapter~\ref{chap:foundation_power} rule-of-thumb that the labeled-sample reduction behaves like a squared-correlation gain.
...
\Var(Y)-NS^2
+
\sqrt{(NS^2-\Var(Y))^2+4S^2\Var(Y)N(1-\rho_{Y,f}^2)}
}
{2S^2}.
\]
This makes the design logic explicit: stronger predictions, represented by larger $\rho_{Y,f}^2$, shift the required labeled sample size downward.
