...
Chapter~\ref{chap:foundation_llm} uses the efficient-influence-function perspective for mean estimation with a surrogate label. Let $R$ indicate whether the gold-standard outcome $Y$ is observed, with
\[
\Pr(R=1)=\pi,
\]
and assume that $R$ is independent of $(Y,\widehat{Y})$. Define
\[
...
\theta=\mathbb{E}[Y]
=
\mathbb{E}\{\mu(\widehat{Y})\}+\mathbb{E}\{Y-\mu(\widehat{Y})\}.
\]
...
\]
Standard PPI corresponds to the working approximation $\mu(\widehat{Y})\approx \widehat{Y}$, while linear PPI++ corresponds to $\mu(\widehat{Y})\approx \lambda \widehat{Y}$.

\section{Rogan--Gladen Algebra for the Binary Misclassification Model}

Suppose $Y,\widehat{Y}\in\{0,1\}$ and define
\[
\theta = \Pr(Y=1),\qquad
q_0=\Pr(\widehat{Y}=0\mid Y=0),\qquad
q_1=\Pr(\widehat{Y}=1\mid Y=1),
\]
along with the observed surrogate prevalence
\[
p=\Pr(\widehat{Y}=1).
\]
...
\section{Mean \texorpdfstring{PPI++}{PPI++} Variance and the Oracle \texorpdfstring{$\lambda$}{lambda}}
...
\]
Define the target variance scale
\[
S^2=
\left(
...
\]
provided $S^2>\lambda^2\Var(f)/N$. This is the direct inversion used for vanilla PPI ($\lambda=1$) and for fixed plug-in PPI++ using an estimated $\widehat{\lambda}$.
