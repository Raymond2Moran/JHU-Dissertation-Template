...
The power-analysis problem asks how many gold-standard labels are needed before the study is run. In the prediction-powered setting, the answer depends not only on the target estimand but also on how closely the surrogate tracks the truth on a labeled pilot subset.
...
The power-analysis preprint provides the planning layer for the rest of the thesis. Once the protein-interaction prediction workflow yields a strong surrogate, the same framework can be used to ask how many experimentally validated edges are needed to estimate a prevalence-like quantity to a desired precision and how those requirements change under vanilla PPI, estimated-lambda \texttt{PPI++}, and oracle-lambda \texttt{PPI++}. Chapter~\ref{chap:protein_results} answers those questions with the final protein surrogate and then extends the discussion to shared-protein dependence.
