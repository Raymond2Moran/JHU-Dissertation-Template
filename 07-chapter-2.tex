...
\theta = \mathbb{E}[Y_i],
\]
while using the large prediction-only sample to reduce variance and the smaller labeled sample to correct bias. For any quantity $A_i$, write
\[
\overline{A}_{U}=\frac{1}{N}\sum_{i\in U}A_i,
\qquad
\overline{A}_{L}=\frac{1}{n}\sum_{i\in L}A_i.
\]
When the surrogate is continuous or model-based, the notation
\[
\mu(\widehat{Y}_i)=\mathbb{E}[Y_i\mid \widehat{Y}_i]
\]
denotes the calibration function linking the cheap label to the gold-standard outcome.

To keep the rest of the thesis consistent, I use the additional shorthand
\[
f_i=f(X_i)
\]
for a generic surrogate prediction. In the LLM-as-a-judge setting, one can read $f_i$ as a working model for the information contained in $\widehat{Y}_i$; in the later protein chapters, $f_i$ is the learned edge-prediction score itself. With that notation, the same estimator formulas can be reused across chapters without changing their statistical meaning.
...
\]
This equation is the bridge between the semiparametric theory and the practical estimators used later in the thesis. Standard PPI is obtained by the working choice $\mu(\widehat{Y}_i) \approx f_i$, whereas PPI++ uses the linear approximation $\mu(\widehat{Y}_i) \approx \lambda f_i$ \parencite{ppi_framework, ppi_pp, llmjudge_eif}. In the binary judge setting, the simple special case $f_i=\widehat{Y}_i$ recovers the most familiar form of the estimator. Detailed algebra for these identities is summarized in Appendix~\ref{chap:appendix-a}.
