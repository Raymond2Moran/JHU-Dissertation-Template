...
#'
#' Simulates data where \eqn{Z \sim \mathrm{Unif}(\{1,\dots,K\})}, \eqn{Y \mid Z \sim N(\mu_Z,\sigma^2)}, and the prediction is exact: \eqn{\hat Y = Z}.
#'
#' @param N Sample size
...
#' 
#' Estimates the target mean by debiasing the test-set predictions using the calibration-set mean correction.
#' \deqn{\hat\theta = \overline{\hat Y}_{\mathrm{test}} + \left(\overline{Y}_{\mathrm{cal}} - \overline{\hat Y}_{\mathrm{cal}}\right).}
#'
...
#' \deqn{\lambda^* = \frac{\mathrm{Cov}\left(Y_{\text{cal}},\hat{Y}_{\text{cal}}\right)}{\mathrm{Var}(\hat{Y}_{\text{cal}}) + \frac{m}{n} \cdot \mathrm{Var}(\hat{Y}_{\text{test}})}}
#'
#' @param Y_cal Vector of true labels on the calibration set.
#' @param Yhat_cal Vector of surrogate predictions on the calibration set.
#' @param Yhat_test Vector of surrogate predictions on the test set.
...
#'
#' Learns \eqn{g: \{1,...,K\} \rightarrow \mathbb{R}} such that \eqn{g(z) = E[Y | \hat{Y} = z]}.
#' For discrete \eqn{\hat{Y}}, this is just the conditional mean per category
#'
#' @param Y_cal Vector of true labels on the calibration set.
#' @param Yhat_cal Vector of surrogate predictions on the calibration set.
#' @param Yhat_test Vector of surrogate predictions on the test set.
...
#'
#' Fits \eqn{g(\hat{Y}) = a + b\cdot\hat{Y}} using linear regression.
#' This is suboptimal when \eqn{\hat{Y}} is discrete but shows importance of proper calibration.
#'
#' @param Y_cal Vector of true labels on the calibration set.
#' @param Yhat_cal Vector of surrogate predictions on the calibration set.
#' @param Yhat_test Vector of surrogate predictions on the test set.
...
#'
#' Fits \eqn{g(\hat{Y})} using a GAM with smooth term.
#' For discrete \eqn{\hat{Y}} with few categories, this behaves similarly to per-category.
#'
#' @param Y_cal Vector of true labels on the calibration set.
#' @param Yhat_cal Vector of surrogate predictions on the calibration set.
#' @param Yhat_test Vector of surrogate predictions on the test set.
...
#'
#' Fits \eqn{g(\hat{Y})} using natural splines.
#' For discrete \eqn{\hat{Y}} with few categories, this behaves similarly to per-category.
#'
#' @param Y_cal Vector of true labels on the calibration set.
#' @param Yhat_cal Vector of surrogate predictions on the calibration set.
...
#' Oracle estimator (knows true \eqn{\mu} values)
#'
#' @param Yhat_test Vector of surrogate predictions on the test set.
