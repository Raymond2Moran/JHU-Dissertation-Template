...
\section[Related Work]{Related Works}
...
The noisy-surrogate extension in \textcite{llmjudge_eif} matters here because it makes the calibration story explicit. It clarifies how prediction-powered correction relates to classical misclassification correction and to efficient-influence-function arguments when the cheap signal is noisy or biased. That perspective is useful well beyond LLM evaluation. It gives the thesis a general language for treating protein-edge prediction scores as inferential surrogates rather than as stand-alone benchmark outputs.
...
Previous studies show that strong single-protein representations are now practically attainable. Moreover, the quality of evaluations of the protein-pairs is highly sensitive to split construction and negative-edge design \parencite{park2012, bernett2024}. To reflect this, we used two datasets in the thesis: the Intra benchmark with its predefined negatives, the BIOGRID-human benchmark with randomly sampled non-positive pairs, and cross-dataset comparisons are interpreted cautiously because the labellings criteria should not be treated equally.


Taken together, this thesis is not only a summary of semiparametric efficiency, not only a benchmark paper on protein-interaction prediction, and not merely a reproduction of recent papers. Its contribution is to connect prediction-powered estimation, prediction-powered planning, and leakage-aware protein-edge modeling into one coherent inferential workflow, and then to examine where that workflow transfers cleanly and where the protein setting needs extra attentions.
