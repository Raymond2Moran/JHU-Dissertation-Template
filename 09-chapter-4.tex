...
The biological task is narrow and concrete: given a pair of proteins, predict whether that pair is treated by the benchmark as a physical protein--protein interaction. Positive edges therefore correspond to curated physical interactions, while negative edges are either benchmark-defined negatives or sampled non-positive pairs. The point here is not to catalog implementation details. It is to show how that biological task is turned into a leakage-aware prediction problem whose output can be carried into the later inferential analysis.

The predictive pipeline then follows a simple sequence. Protein sequences and UniProt text descriptions are encoded into protein-level embeddings, those embeddings are converted into pairwise edge features, several prediction heads are compared, and a fixed three-seed attention ensemble is taken forward as the default surrogate. Figure~\ref{fig:chap4_pipeline_overview} summarizes that workflow. The discussion follows the same order: split design and benchmark construction first, protein representations and pair features second, and the final surrogate handoff last.
