...
This chapter develops the prediction layer used in the protein application. It is not a software walkthrough. Instead, the aim is to describe the scientific data sources, leakage-aware split design, protein representations, pair-feature construction, model families, and final surrogate that are later carried into prediction-powered inference. The predictive path studied here is the sequence-plus-text attention-ensemble workflow, but the chapter is organized around the methodological choices behind that workflow rather than around software internals.

Figure~\ref{fig:chap4_pipeline_overview} summarizes the predictive pipeline at a high level. Protein sequences and UniProt text descriptions are first converted into protein-level embeddings, then transformed into edge-level pair features, and finally passed to a small family of predictive heads. The discussion follows the same order. It begins with split design and benchmark construction, then turns to protein representations and pair features, and finally explains why the frozen three-seed attention ensemble is the surrogate carried forward into the inference chapters.
